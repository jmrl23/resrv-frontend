    ...
    <div>
      <div className='flex flex-col gap-x-2 gap-y-4 md:flex-row justify-between border-b border-b-gray-200 pb-4'>
        <div className='flex justify-end w-full'>
          <button
            className='btn btn-blue flex gap-x-2'
            type='button'
            title='register'
...
export const ModeratorsTableRow: FC<{
  userLevel: UserLevel
  mutate: () => void
  setModal: Dispatch<
    SetStateAction<{
      ...
      <td>
        ...
        >
          {Object.keys(Role).map(
            (role) =>
              role !== Role.STUDENT && <option key={role}>{role}</option>
          )}
        </select>
      </td>
      <td>
...
export const ModeratorsCreateModal: FC<{
  hide: () => void
  mutate: () => void
}> = ({ hide, mutate }) => {
  const [cookies] = useCookies(['session'])
    ...
    <Modal hide={hide}>
      ...
      >
        <header className='flex justify-between items-center'>
          <h2 className='font-poppins text-xl font-bold'>Register moderator</h2>
          <button
        ...
        <section className='flex flex-col gap-y-4'>
          <div>
            <label
              className='font-bold'
              htmlFor='register-moderator-modal-email'
            >
              Email
            </label>
            <input
              ...
              type='email'
              id='register-moderator-modal-email'
              name='email'
              placeholder='jad@paterostechnologicalcollege.edu.ph'
            />
          </div>
          <div>
            <label
              className='font-bold block'
              htmlFor='edit-department-modal-name'
            >
              Role
            </label>
            <select
...
export const ModeratorsDeleteModal: FC<{
  hide: () => void
  mutate: () => void
  ...
  return (
    <Modal hide={hide}>
      ...
      >
        <header className='flex justify-between items-center'>
          <h2 className='font-poppins text-xl font-bold'>Delete Moderator</h2>
          <button
            className='p-2 rounded-lg bg-gray-100 hover:bg-gray-200'
            type='button'
            title='close'
