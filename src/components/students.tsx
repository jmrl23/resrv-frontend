...
export const Students: FC<Record<string, never>> = () => {
  ...
  const studentFetcher = useCallback(async () => {
    ...
    if (departmentId && departmentId !== 'placeholder') payload.departmentId = departmentId
  ...
  const handleSearchSubmit: FormEventHandler<HTMLFormElement> = useCallback((event: FormEvent) => {
  ...
  return (
    <div>
      <div className="flex flex-col gap-x-2 gap-y-4 md:flex-row justify-between border-b border-b-gray-200 pb-4">
        <form className="flex gap-x-2 w-full md:max-w-screen-sm" ref={searchFormRef} onSubmit={handleSearchSubmit}>
          <input className="border-none rounded-md bg-gray-200 px-4 py-2 w-[calc(100%-4rem)]" placeholder="Search" type="search" name="q" ref={searchInputRef} />
          <button className="btn w-14" type="submit">
            <MagnifyingGlassIcon className='w-6 h-6' />
          </button>
        </form>
        <div className="flex justify-end gap-x-2">
          <button className="btn" type="button" title="previous" onClick={() => {
            setSkip(value => {
              if (!value) return 0
          ...
          <select className="border-none bg-gray-200 rounded-md text-sm grow md:-order-1" title="Filter by Department" ref={filterDepartmentRef} onChange={() => mutate('/api/user/list')}>
            <option value="placeholder" defaultChecked>Filter by department</option>
            {
              departments?.map((department) => (
                <option key={department.id} value={department.id}>{department.alias}</option>
              ))
            }
          </select>
          <button className="btn" type="button" title="next" onClick={() => {
            setSkip(value => {
      ...
      <div className="overflow-x-auto mt-4">
        <table>
          ...
          <tbody>
            {
              !isLoading && students?.map(student => <TableRow key={student.id} student={student} mutate={() => mutate('/api/user/list')} setModal={setModal} />)
            }
          </tbody>
        </table>
      </div>
...
export const TableRow: FC<{ 
  ...
  return (
    <tr>
      ...
      <td>{student.StudentInformation?.department?.id ?? <span className='text-gray-400'>N/A</span>}</td>
      <td>
        <div className='flex gap-x-2'>
          ...
          <button type='button' className='btn btn-red' onClick={() => setModal({ for: 'delete', data: student })}>
            Delete
          </button>
        </div>
      </td>
